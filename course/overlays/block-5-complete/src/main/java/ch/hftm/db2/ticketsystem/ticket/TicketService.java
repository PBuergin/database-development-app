...
class TicketService {
    ...
    @Transactional(readOnly = true)
    List<TicketListItemResponse> findTicketOverview(String status) {
        String normalizedStatus = StringUtils.hasText(status) ? normalizeStatus(status) : null;
        return ticketRepository.findActiveTicketOverview(normalizedStatus);
    }

    @Transactional(readOnly = true)
    List<TicketStatusPriorityReportResponse> findStatusPriorityReport() {
        return ticketRepository.findActiveStatusPriorityReport().stream()
                .map(row -> new TicketStatusPriorityReportResponse(
                        row.getStatus(),
                        row.getPriority(),
                        row.getTicketCount(),
                        row.getCommentCount()
